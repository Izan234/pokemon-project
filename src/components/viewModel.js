import { Player, PokemonList, PokemonTeam } from "../components/model.js"; // class PokemonList {...}
// class PokemonTeam {...}
//
// =========================
...
export class PokemonTeamViewModel {
  ...
  autoSelectCpuTeam() {
    ...
    this.addToBattleLog(`✅ CPU team selected: ${cpuTeam.getTeamDetails()}`);
  }
  ...
  fightRound() {
    return new Promise((resolve) => {
      ...
      this.addToBattleLog(`⚔️ ${pokemon1.name} vs ${pokemon2.name}`); // cuando haga el battle log esto tiene que poner this.addToBattleLog
      ...
      setTimeout(() => {
        if (pokemon1.special_power == pokemon2.special_power) {
          this.addToBattleLog(
            `💥 ${pokemon1.name} i ${pokemon2.name} es derroten mútuament!`,
            "p",
            true
          );
          ...
          let message = this.player1.team.decreaseSpecialPower(
            pokemon1.name,
            damageMade
          );
          ...
          let message = this.player2.team.decreaseSpecialPower(
            pokemon2.name,
            damageMade
          );
          this.addToBattleLog(`${message}`, "p", true);
  ...
  getRandomFighter(team) {
    ...
    this.getQuienLucha(team.selectedTeam[randomIndex].fight());
    return team.selectedTeam[randomIndex];
  }
  getQuienLucha(player) {
    switch (player) {
      case "Player1":
        let whoIsFighting1 = this.player1.getQuienLucha();
        console.log(whoIsFighting1);
        return whoIsFighting1;
      //        return this.player1.getQuienLucha();
      case "Player2":
        let whoIsFighting2 = this.player2.getQuienLucha();
        console.log(whoIsFighting2);
        return whoIsFighting2;
      //        return this.player2.getQuienLucha();
    }
  }
